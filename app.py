...
with col1:
    ...
    # Товщина лінії 25 краще імітує маркер з датасету MNIST
...
with col2:
    ...
    if canvas_result.image_data is not None:
        ...
        if st.button("Розпізнати цифру"):
            # Перевіряємо, чи полотно не порожнє
            if cv2.countNonZero(gray_image) == 0:
                st.warning("Спочатку намалюйте цифру!")
            else:
                # --- РОЗУМНА ОБРОБКА ЗОБРАЖЕННЯ ---
                # 1. Знаходимо межі намальованої цифри (Bounding Box)
                coords = cv2.findNonZero(gray_image)
                x, y, w, h = cv2.boundingRect(coords)
                digit = gray_image[y:y+h, x:x+w]

                # 2. Масштабуємо так, щоб більша сторона була 20 пікселів (стандарт MNIST)
                if w > h:
                    new_w = 20
                    new_h = int((20 / w) * h)
                else:
                ...
                # Використовуємо INTER_AREA для кращого стиснення без втрат
                resized_digit = cv2.resize(digit, (new_w, new_h), interpolation=cv2.INTER_AREA)

                # 3. Додаємо чорні поля, щоб розмір став 28x28, а цифра була строго по центру
                pad_top = (28 - new_h) // 2
                ...
                # Нормалізуємо дані (0-1) та змінюємо форму для CNN
